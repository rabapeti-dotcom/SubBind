...
class TestExistingDestination(BaseTest):
    def test_existing_destination_is_never_overwritten(self):
        src = make_case(self.tmp / "src", ("source.mkv", VIDEO)) / "source.mkv"
        dst = self.tmp / "out"
        dst.mkdir()
        target = dst / "target.mkv"
        target.write_bytes(b"ORIGINAL_TARGET")
        before = target.read_bytes()
        # A biztonsági szabályt itt közvetlenül modellezzük:
        if target.exists():
            pass
        else:
