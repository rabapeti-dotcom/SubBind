...
class TestOriginalUntouched(BaseTest):
    def test_source_digest_survives_copy(self):
        src = make_case(self.tmp / "src", ("video.mkv", VIDEO)) / "video.mkv"
        digest = sha256(src)
        out = self.tmp / "new_output"
        out.mkdir()
        shutil.copy2(src, out / "video_new.mkv")
