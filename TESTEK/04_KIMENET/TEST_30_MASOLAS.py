...
class TestCopyOutput(BaseTest):
    def test_copy_to_destination_keeps_source(self):
        src = make_case(self.tmp / "src", ("Show.S01E01.mkv", VIDEO)) / "Show.S01E01.mkv"
        dst = self.tmp / "out"
        digest = sha256(src)
        dst.mkdir()
        shutil.copy2(src, dst / "Show.S01E01.mkv")
        self.assertTrue((dst / "Show.S01E01.mkv").exists())
        self.assert_same_file(src, digest)
