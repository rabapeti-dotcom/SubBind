...
class TestAmbiguousEpisode(BaseTest):
    def test_bare_episode_does_not_claim_high_confidence(self):
        p = make_case(self.tmp, ("Show.E05.mkv", VIDEO)) / "Show.E05.mkv"
        item = parse(p)
        self.assertFalse(item.confidence == "high" and item.season is not None)
