...
class TestPairSelection(BaseTest):
    def test_pair_contains_video_and_subtitle(self):
        make_case(self.tmp,
                  ("Show.S01E01.mkv", VIDEO),
                  ("Show.S01E01.hu.srt", SUB))
        files = [p for p in self.tmp.iterdir()]
        kinds = {parse(p).kind for p in files}
        self.assertEqual(kinds, {"video", "sub"})
