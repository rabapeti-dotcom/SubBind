...
class TestMultipleHungarianSubtitles(BaseTest):
    def test_variants_can_be_detected_separately(self):
        make_case(self.tmp,
                  ("Show.S01E01.mkv", VIDEO),
                  ("Show.S01E01.hu.srt", SUB),
                  ("Show.S01E01.hu.forced.srt", SUB))
        subs = [parse(self.tmp / n) for n in
                ("Show.S01E01.hu.srt", "Show.S01E01.hu.forced.srt")]
        self.assertTrue(all(x.kind == "sub" for x in subs))
