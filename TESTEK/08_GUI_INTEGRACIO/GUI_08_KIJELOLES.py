...
class GuiBase(unittest.TestCase):
    def setUp(self):
        ...
        self.data_dir = Path(tempfile.mkdtemp(prefix="sr_gui08_data_"))
        self.out_dir = Path(tempfile.mkdtemp(prefix="sr_gui08_out_"))
...
class TestGuiSelection(GuiBase):
    def test_select_all_none_and_missing_are_real_gui_state_changes(self):
        src = make_case(self.tmp,
                        ("Show.S01E01.mkv", VIDEO),
                        ("Show.S01E01.hu.srt", SUB))
        self.add(*(src / n for n in ("Show.S01E01.mkv", "Show.S01E01.hu.srt")))
