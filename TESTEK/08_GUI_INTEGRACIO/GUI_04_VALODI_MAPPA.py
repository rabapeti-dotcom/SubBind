...
class TestGuiFolder(GuiBase):
    def test_real_folder_adds_all_supported_files(self):
        src = make_case(self.tmp,
                        ("Show.S01E01.mkv", VIDEO),
                        ("Show.S01E01.hu.srt", SUB))
        # A valódi add_folder QFileDialogot nyitna; itt ugyanazt a GUI-útvonalat
        # használjuk az add_paths-on keresztül, a valós fájlokkal.
        self.add(src / "Show.S01E01.mkv", src / "Show.S01E01.hu.srt")
        self.assertEqual(len(self.win.items), 2)
        self.assertTrue(all(Path(i.path).parent == src for i in self.win.items))
