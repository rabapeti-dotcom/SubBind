...
class TestSingleVideo(BaseTest):
    def test_video_is_parsed_as_video(self):
        p = make_case(self.tmp, ("Film.Cime.2024.mkv", VIDEO)) / "Film.Cime.2024.mkv"
        item = parse(p)
        self.assertEqual(item.kind, "video")
        self.assertTrue(item.ext.lower() in {".mkv", "mkv"})
