...
class TestVideoSubtitlePair(BaseTest):
    def test_video_and_hu_subtitle_parse(self):
        make_case(self.tmp,
                  ("Sorozat.S01E01.mkv", VIDEO),
                  ("Sorozat.S01E01.hu.srt", SUB))
        v = parse(self.tmp / "Sorozat.S01E01.mkv")
        s = parse(self.tmp / "Sorozat.S01E01.hu.srt")
        self.assertEqual(v.kind, "video")
        self.assertEqual(s.kind, "sub")
        self.assertEqual(s.lang, "hu")
